<?php
/*
  Plugin Name: WooCommerce pz Gateway
  
  Description: Extends WooCommerce with pz Gateway.
  Version: 1.0
  Author: pz
...
if (!defined('ABSPATH'))
    exit;
add_action('plugins_loaded', 'woocommerce_pz_init', 0);

function woocommerce_pz_init() {
     ...
     * Gateway class
     */
    class WC_PZ extends WC_Payment_Gateway {
        ...
        public function __construct() {
           ...
           // $this->testurl = $this->settings['testurl'];
            ...
            $this->notify_url = str_replace('https:', 'http:', home_url('index.php/checkout/wc-api/WC_PZ'));
            $this->msg['message'] = "";
            $this->msg['class'] = "";

            //update for woocommerce >2.0
            add_action('woocommerce_api_wc_pz', array($this, 'check_pz_response'));
            add_action('valid-pz-request', array($this, 'successful_request'));
            if (version_compare(WOOCOMMERCE_VERSION, '2.0.0', '>=')) {
                add_action('woocommerce_update_options_payment_gateways_' . $this->id, array($this, 'process_admin_options'));
            } else {
                add_action('woocommerce_update_options_payment_gateways', array(&$this, 'process_admin_options'));
            }
            add_action('woocommerce_receipt_pz', array($this, 'receipt_page'));
            add_action('woocommerce_thankyou_pz', array($this, 'thankyou_page'));
        }
        ...
        function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('Enable Pz Payment Module.', 'pz'),
                    'default' => 'no'),
                'title' => array(
                    'title' => __('Title:', 'pz'),
                    'type' => 'text',
                ...
                'description' => array(
                    ...
                    'default' => __('Pay securely by Credit or internet banking through Pz Secure Servers.', 'pz')),
                'merchant_id' => array(
                    'title' => __('Merchant ID', 'pz'),
                    'type' => 'text',
                    'description' => __('This id(USER ID) available at "Generate Working Key" of "Settings and Options at pz."')),
                'working_key' => array(
                    'title' => __('Working Key', 'pz'),
                    'type' => 'text',
                    'description' => __('Given to Merchant by pz', 'pz'),
                ),
                'totype' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('Pz Payment Gateway', 'pz') . '</h3>';
            echo '<p>' . __('Pz is most popular payment gateway for online shopping') . '</p>';
        ...
        /**
         *  There are no payment fields for pz, but we want to show the description if set.
         * */
        ...
        function receipt_page($order) {
            ...
            echo '<p>' . __('Thank you for your order, please click the button below to pay with Pz.', 'pz') . '</p>';
            echo $this->generate_pz_form($order);
        }
        ...
        /**
         * Check for valid pz server callback
         * */
        function check_pz_response() {
            global $woocommerce;
            $msg['class'] = 'error';
                        ...
                        if (order_status != '') {
                            if ($order_status == "Y") {
                                ...
                                $msg['class'] = 'success';
                                    ...
                                    global $wpdb;
                                    $wpdb->update('wp_pz_tbl', array('order_status' => 'capturesuccess','tracking_id'=>$trackingid), array('woocommerce_id' => $first));
                                ...
                                global $wpdb;
                                $wpdb->update('wp_pz_tbl', array('order_status' => 'authstarted','tracking_id'=>$trackingid), array('woocommerce_id' => $first));
                                $woocommerce->cart->empty_cart();
                                ...
                                global $wpdb;
                                $wpdb->update('wp_pz_tbl', array('order_status' => 'authfailed','tracking_id'=>$trackingid), array('woocommerce_id' => $first));
                                $woocommerce->cart->empty_cart();
                            } else {
                                $msg['class'] = 'error';
                                $msg['message'] = "Thank you for shopping with us. However, the transaction has been declined.";
        ...
        /**
         * Generate pz button link
         * */
        public function generate_pz_form($order_id) {
            global $woocommerce;
            $order = new WC_Order($order_id);
			...
			//die();
            //break;
            foreach ($pz_args as $param => $value) {
            ...
            $pz_args_array[] = "<input type='hidden' name='reservedField1' value='{$this->reservedField1}'/>";
            $pz_args_array[] = "<input type='hidden' name='reservedField2' value='{$this->reservedField2}'/>";
            $pz_args_array[] = "<input type='hidden' name='totype' value='{$this->totype}'/>";
            $pz_args_array[] = "<input type='hidden' name='pctype' value='1_1|1_2'/>";
            $pz_args_array[] = "<input type='hidden' name='amount' value='{$order->order_total}'/>";
            $pz_args_array[] = "<input type='hidden' name='orderdescription' value='{$order_id}'/>";
            $pz_args_array[] = "<input type='hidden' name='description' value='{$order_id}'/>";
            $pz_args_array[] = "<input type='hidden' name='redirecturl' value='{$redirecturl}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_street' value='{$order->billing_address_1}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_state' value='{$order->billing_state}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_city' value='{$order->billing_city}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_zip' value='{$order->billing_postcode}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_telno' value='{$order->billing_phone}'/>";
			
			$country_value = $country_code[$country];
			$pz_args_array[] = "<input type='hidden' name='telnocc' value='{$country_value }'/>";
			
            $pz_args_array[] = "<input type='hidden' name='TMPL_emailaddr' value='{$order->billing_email}'/>";
            $pz_args_array[] = "<input type='hidden' name='language' value='{get_locale()}'/>";
            $pz_args_array[] = "<input type='hidden' name='ipaddr' value='{$ip}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_COUNTRY' value='{$country}'/>";
            $pz_args_array[] = "<input type='hidden' name='TMPL_CURRENCY' value='{$CURRENCY}'/>";
            $pz_args_array[] = "<input type='hidden' name='currency' value='{$CURRENCY}'/>";
            $pz_args_array[] = "<input type='hidden' name='checksum' value='{$checksum}'/>";
            //$pz_args_array[] = "<input type='hidden' name='terminalid' value='2055'/>";
...
jQuery("#submit_pz_payment_form").click();
');

            $form = '<form action="' . esc_url($this->url) . '" method="post" id="pz_payment_form" target="_top">
' . implode('', $pz_args_array) . '
<!-- Button Fallback -->
<div class="payment_buttons">
<input type="submit" class="button alt" id="submit_pz_payment_form" value="' . __('Pay via Pz WooCommerce', 'woocommerce') . '" /> <a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . __('Cancel order &amp; restore cart', 'woocommerce') . '</a>
</div>
...
jQuery(".payment_buttons").hide();
     ...
     * */
    function woocommerce_add_pz_gateway($methods) {
        $methods[] = 'WC_PZ';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_pz_gateway');
}
...
register_activation_hook(__FILE__, 'create_pz_table');
/**
...
function my_plugin_settings() {
    add_menu_page('pz', 'Reconciliation', 'administrator', 'insert-my-meta', 'my_plugin_settings_page', 'dashicons-filter', '54');
}
...
function my_plugin_settings_page() {
   ...
   // echo '</div>';
    ...
    $total = $wpdb->get_var("SELECT COUNT(`id`) FROM wp_pz_tbl where order_status = 'authstarted' " );
    $num_of_pages = ceil($total / $limit);

    /*  Paging ends here  */
    $fivesdrafts = $wpdb->get_results("SELECT * FROM wp_pz_tbl WHERE order_status = 'authstarted' LIMIT $offset,$limit");
    if ($fivesdrafts) {
